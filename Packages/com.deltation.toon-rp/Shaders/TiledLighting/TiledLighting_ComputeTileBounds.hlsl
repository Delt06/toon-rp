...
void CS(uint3 dispatchThreadId : SV_DispatchThreadID)
{
    ...
    float3 cornersViewSpaceNear[4];
    float3 cornersViewSpaceFar[4];

    UNITY_UNROLL
    for (uint i = 0; i < 4; i++)
    {
        float2 pixelCoords = (float2) cornerPixelCoords[i];
        cornersViewSpaceNear[i] = TiledLighting_ScreenToView(float4(pixelCoords, UNITY_NEAR_CLIP_VALUE, 1.0f)).xyz;
        cornersViewSpaceFar[i] = TiledLighting_ScreenToView(float4(pixelCoords, UNITY_RAW_FAR_CLIP_VALUE, 1.0f)).xyz;
    }
